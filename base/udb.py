...
UCD - Covers built-in unicodedata module with matching functions that
      return None in case of Exceptions.

NOTE: Everything here is HIGHLY EXPERIMENTAL, and could change soon.
      (This note will disappear when I've discovered the way I want
      to do things.)
"""
...
class PropList(object):
	...
	@classmethod
	def keys(cls):
		"""Returns a generator for the property list keys."""
		for k in PROPLIST:
			yield k
	...
	def items(cls, key):
		"""
		Returns a generator for ord integers in the property list 'key'.
		"""
		for item in PROPLIST[key]:
	...
	def hasproperty(cls, c, propname):
		"""Return True if character c has property propname."""
		for item in PROPLIST[propname]:
			if isinstance(item, int) and (item==ord(c)):
				return True
			elif c in range(*item):
				return True
		return False
	...
	def whitespace(cls):
		try:
			...
			items = cls.items('White_Space')
			cls.__WHITESPACE = u''.join([unichr(x) for x in items])
			return cls.__WHITESPACE
	
	
	
	# *** EXPERIMENTAL - For performance comparisons ***
	
	@classmethod
	def iswhitespace(cls, c):
		return cls.hasproperty('White_Space')
	
	@classmethod
	def isasciihex(cls, c):
		return cls.hasproperty('ASCII_Hex_Digit')
	
	@classmethod
	def isbidicontrol(cls, c):
		return cls.hasproperty('Bidi_Control')
	
	@classmethod
	def isdash(cls, c):
		return cls.hasproperty('Dash')
	
	@classmethod
	def isdeprecated(cls, c):
		return cls.hasproperty('Deprecated')
	
	@classmethod
	def isdiacritic(cls, c):
		return cls.hasproperty('Diacritic')
	
	@classmethod
	def isextender(cls, c):
		return cls.hasproperty('Extender')
	
	@classmethod
	def ishexdigit(cls, c):
		return cls.hasproperty('Hex_Digit')
	
	@classmethod
	def ishyphen(cls, c):
		return cls.hasproperty('Hyphen')
